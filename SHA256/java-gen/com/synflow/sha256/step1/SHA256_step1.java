...
final public class SHA256_step1 implements Entity {
	...
	public void execute() {
		...
		case 1:
			if (msg.available()) {
				int local_t = 0;
				msg_in = msg.peekInt(); // (line 25)
				local_t = t; // (line 0)
				isSchedulable = local_t < 0x10;
			}
			if (isSchedulable) {
				// action SHA256_step1_1_a (line 25)
				int local_msg = 0;
				int local_t = 0;
				int local_t0 = 0;
				int local_W = 0;
				int local_t1 = 0;
				int local_t2 = 0;
				msg_in = msg.readInt(); // (line 25)
				local_msg = msg_in; // (line 25)
				local_t = t; // (line 25)
				W[((local_t) & 0x3f)] = local_msg; // (line 25)
				local_t0 = t; // (line 26)
				local_t1 = t; // (line 26)
				local_W = W[((local_t1) & 0x3f)]; // (line 26)
				System.out.println(this + ": " + "W[" + "0x" + Integer.toHexString(local_t0) + "] = " + "0x" + Integer.toHexString(local_W)); // (line 26)
				local_t2 = t; // (line 0)
				t = ((((local_t2) & 0x7f) + 0x1) & 0x7f); // (line 24)
		...
		case 4:
			...
			if (isSchedulable) {
				...
				local_H_i7 = H_i[0x0]; // (line 78)
				System.out.println(this + ": " + "H_i[0] = " + "0x" + Integer.toHexString(local_H_i7)); // (line 78)
				local_H_i8 = H_i[0x1]; // (line 79)
				System.out.println(this + ": " + "H_i[1] = " + "0x" + Integer.toHexString(local_H_i8)); // (line 79)
				local_H_i9 = H_i[0x2]; // (line 80)
				System.out.println(this + ": " + "H_i[2] = " + "0x" + Integer.toHexString(local_H_i9)); // (line 80)
				local_H_i10 = H_i[0x3]; // (line 81)
				System.out.println(this + ": " + "H_i[3] = " + "0x" + Integer.toHexString(local_H_i10)); // (line 81)
				local_H_i11 = H_i[0x4]; // (line 82)
				System.out.println(this + ": " + "H_i[4] = " + "0x" + Integer.toHexString(local_H_i11)); // (line 82)
				local_H_i12 = H_i[0x5]; // (line 83)
				System.out.println(this + ": " + "H_i[5] = " + "0x" + Integer.toHexString(local_H_i12)); // (line 83)
				local_H_i13 = H_i[0x6]; // (line 84)
				System.out.println(this + ": " + "H_i[6] = " + "0x" + Integer.toHexString(local_H_i13)); // (line 84)
				local_H_i14 = H_i[0x7]; // (line 85)
				System.out.println(this + ": " + "H_i[7] = " + "0x" + Integer.toHexString(local_H_i14)); // (line 85)
				local_H_i15 = H_i[0x0]; // (line 88)
				local_H_i16 = H_i[0x1]; // (line 89)
				local_H_i17 = H_i[0x2]; // (line 90)
				local_H_i18 = H_i[0x3]; // (line 91)
				local_H_i19 = H_i[0x4]; // (line 92)
				local_H_i20 = H_i[0x5]; // (line 93)
				local_H_i21 = H_i[0x6]; // (line 94)
				local_H_i22 = H_i[0x7]; // (line 95)
				hash_out = (BigInteger.valueOf(local_H_i15).and(new BigInteger(new byte[] {0, -1, -1, -1, -1})).shiftLeft(0xe0)).or(BigInteger.valueOf(local_H_i16).and(new BigInteger(new byte[] {0, -1, -1, -1, -1})).shiftLeft(0xc0)).or(BigInteger.valueOf(local_H_i17).and(new BigInteger(new byte[] {0, -1, -1, -1, -1})).shiftLeft(0xa0)).or(BigInteger.valueOf(local_H_i18).and(new BigInteger(new byte[] {0, -1, -1, -1, -1})).shiftLeft(0x80)).or(BigInteger.valueOf(local_H_i19).and(new BigInteger(new byte[] {0, -1, -1, -1, -1})).shiftLeft(0x60)).or(BigInteger.valueOf(local_H_i20).and(new BigInteger(new byte[] {0, -1, -1, -1, -1})).shiftLeft(0x40)).or(BigInteger.valueOf(((((long) (local_H_i21)) & 0xffffffffL) << 0x20)).and(new BigInteger(new byte[] {0, -1, -1, -1, -1, -1, -1, -1, -1}))).or(BigInteger.valueOf(local_H_i22).and(new BigInteger(new byte[] {0, -1, -1, -1, -1}))); // (line 87)
				hash.write(hash_out);
